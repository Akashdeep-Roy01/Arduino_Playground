...
//PID Definition
double Setpoint;  
volatile double Output, PressureReading, InputFlowReading, OutputFlowReading; 
...
int ReadActualInputFlowValue(){
  ...
  int result = lowpassFilter.input(convertvolttoflow*temp - constantflow - zero_correctioninputflow);
  return result;
}
int ReadActualOutputFlowValue(){
  int temp = analogRead(OutputFlowSensor_Pin);
  int result = lowpassFilter.input(convertvolttoflow*temp - constantflow - zero_correctionoutputflow);
  return result;
}
...
int CheckZeroError(){
  ...
  float sampleaverage = totalsample/(3*i);
  if(sampleaverage >0.3){
    ...
    Serial.println(zero_correctioninputflow);
    Serial.println(zero_correctionoutputflow);
    } 
  }
...
void plotGraph(){
  ...
  InputFlowReading = lowpassFilter.input(convertvolttoflow*(analogRead(InputFlowSensor_Pin)) - constantflow - zero_correctioninputflow);
  OutputFlowReading = lowpassFilter.input(convertvolttoflow*(analogRead(OutputFlowSensor_Pin)) - constantflow - zero_correctioninputflow);
  int FlowReading = InputFlowReading - abs(OutputFlowReading)+ 60;
  ...
  int result = lowpassFilter.input(convertvolttopressure*yy - constantpressure - zero_correctionpressure)+ 10;
  //Serial.println(yy);
  ...
  Serial1.write(0xff);
  Serial1.write(0xff);
  Serial1.print("add 15,0," + String(FlowReading));
  Serial1.write(0xff);
  Serial1.write(0xff);
  Serial1.write(0xff);
